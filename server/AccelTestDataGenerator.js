...
class AccelTestDataGenerator{
	...
	SendNewPacketToGS(){
		...
		payload.push.apply(payload,bin.float32ToBytes(20.345,true)); //Raw Y Axis data
		payload.push.apply(payload,bin.float32ToBytes(20.678,true)); //Raw Z Axis data
		payload.push.apply(payload,bin.float32ToBytes(50,true)); //Pitch Angle
		payload.push.apply(payload,bin.float32ToBytes(60,true)); //Roll Angle
